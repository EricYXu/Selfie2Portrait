### ABOUT THIS PROGRAM: Tinker with UniHuman's intermediate steps to try and better preserve facial identity!

## TODO: Exploit code in UniHuman
## 1. Find the textures that are produced
...
## SELF-INSERTED HELPER CODE AND FUNCTIONS!!!
# NOTE: Lines 385, 506 changed to fulfill testing purposes

# Self implemented way to extract pwarped_tex_img_np --> np.array variable
pwarped_tex_img_np_extract = np.zeros((1668, 2500, 3), dtype=np.uint8)
...
if __name__=='__main__':
    ...
    if opt.task=='reposing':
        ...
        for src_path,tgt_path in tqdm(zip(src_img_paths,tgt_img_paths)):
            ...
            #   Image.fromarray(result).save(osp.join(opt.out_dir,out_name)) # NORMAL UNIHUMAN SCRIPT
            #   Image.fromarray(pwarped_tex_img_np_extract).save(osp.join(opt.out_dir,out_name)) # SELF-INSERTED COMMAND TO SAVE BODY TEXTURE ARRAY
